...
const getSpeakerAvatarUrl = (speaker?: EventSpeaker) => {
  ...
  if (speaker.avatar_url) {
    if (speaker.avatar_url.startsWith('http')) {
      ...
      return `http://127.0.0.1:8000${speaker.avatar_url}`;
    }
  }
...
const Events = () => {
  ...
  const testApiEndpoints = async () => {
    const endpoints = [
      { name: 'Events', url: 'http://127.0.0.1:8000/api/events/events/' },
      { name: 'Event Types', url: 'http://127.0.0.1:8000/api/events/event-types/' },
      { name: 'Registrations', url: 'http://127.0.0.1:8000/api/events/event-registrations/' },
    ];
...
const handleFormRegistration = async (formData: any) => {
  ...
  try {
    ...
    const response = await fetch('http://127.0.0.1:8000/api/events/event-registrations/', {
      method: 'POST',
      headers: {
  ...
  const checkRegistrationStatus = async (eventId: number, email: string) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/events/event-registrations/check/?event=${eventId}&email=${encodeURIComponent(email)}`
      );
